...
st.sidebar.title("Model on Influenzia DataSet")
cases=st.sidebar.number_input('Enter Number of Cases For The Current Day for Influencezia',value=0)
weektoforecast = st.sidebar.number_input('Enter Number of Weeks to forecast',min_value=1)
...
df=pd.read_csv('C:\\Users\\yousuf\\influenza_updated.csv')
...
loaded_model=load_model('fiver.h5')
...
st.title("Predicted and Test Data Comparision")
st.line_chart(test)


st.write("RMSE: "+str(np.sqrt(mean_squared_error(test_scaled,test_predictions))))
